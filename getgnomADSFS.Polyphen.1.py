...
# LT 8/09/2020
# use polyphen score to weight missense variants
...
# get polyphen
ht = ht.annotate(polyphen = ht.vep.transcript_consequences.find(lambda x: x.consequence_terms.contains('missense_variant') & (x.biotype == 'protein_coding') & (x.gene_symbol_source == 'HGNC')).polyphen_score)
ht = ht.annotate(polyphen = hl.if_else(hl.is_defined(ht.polyphen), ht.polyphen, hl.literal(1.0)))

# bin allele counts per octave
ht = ht.annotate(cat = hl.floor(hl.log(ht.freq[0].AC,2)))

# group by gene and summarize 
res = ht.group_by('gene','cat').aggregate(obs=hl.agg.count(), poly=hl.agg.sum(ht.polyphen), an=hl.agg.sum(ht.freq[0].AN))

res.export('gs://vccrieg/optiRVIS/gnomADSFS.Polyphen.2.tsv')
