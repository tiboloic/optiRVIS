...
approxps = function(beta, coefs) {
  res = apply(coefs, 1, function(l) (l[1]+l[2]*beta+l[3]*beta^2+l[4]*beta^3)/(1+l[5]*beta+l[6]*beta^2+l[7]*beta^3))
  res = exp(res)
  res = res/sum(res)
  res
}
...
SFSplot = function(i) {
  ...
  plot(as.numeric(dat[i,2:19]), log='y', main=dat$gene[i], xlab='octave', ylab='# variants')
  lines(approxps(beta + sig * dat$beta[i], coefs) * sum(as.numeric(dat[i,2:19])), col=2, lwd=2)
  lines(approxps(beta, coefs) * sum(dat[i,2:19]), col=1, lwd=1)
}
...
sapply(match(set2$gene[order(set2$beta, decreasing = F)[1:17]], dat$gene), SFSplot)


# plot of all mutations on arithmetic scale
beta=f$env$parList()$b
sig = exp(f$env$parList()$lsigb)
allSFS = as.matrix(dat[,2:19])
bxs = barplot(colSums(allSFS), log='', main='', xlab='octave', ylab='# variants')
