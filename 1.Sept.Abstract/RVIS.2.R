...
# do chromosomes instead of genes
res = list()
#for (i in 1:(length(batches)-1)) {
#for (i in 14:(length(batches)-1)) {
for (i in 2:nrow(contigs)) {
  chrom = GRanges(rownames(contigs)[i], IRanges(1,as.integer(contigs$length[i])))
#cat(paste("batch ", i, "\n"))
cat(paste("chromosome ", i, "\n"))
#  res[[i]] = countonegene(genes[batches[i]:(batches[i+1]-1)])
  res[[i]] = countonegene(chrom)
}
