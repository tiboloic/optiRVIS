...
optiRVIS = inner_join(optiRVIS, gevir, by=c('gene'='gnomad_gene_name'))
...
lines(pcdenovo ~ pcl, data=counts, lwd=2, col=cols[7])

counts = orderpercent(optiRVIS, "pm", c("denovo", "l"))
lines(pcdenovo ~ pcl, data=counts, lwd=2, col=cols[8])
AUC.pwloeuf = AUC(counts$pcl, counts$pcdenovo)

counts = orderpercent(optiRVIS, "beta", c("denovo", "l"))
lines(pcdenovo ~ pcl, data=counts, lwd=2, col=cols[9])

counts = orderpercent(optiRVIS, "oe_lof_upper", c("denovo", "l"))
lines(pcdenovo ~ pcl, data=counts, lwd=2, col=cols[10])
AUC.loeuf = AUC(counts$pcl, counts$pcdenovo)

counts = orderpercent(optiRVIS, "gevir_percentile", c("denovo", "l"))
lines(pcdenovo ~ pcl, data=counts, lwd=2, col=cols[11])

legend('bottomright', legend = c('RVIS no filter', 'RVIS 1 %', 'RVIS 0.1 %', 'RVIS 0.01 %', 'RVIS 0.001 %', 'Rare RVIS', 'Ideal', 'powerLOEUF', 'powerSFS', 'LOEUF', 'Gevir'), fill=cols[1:11], bty='n')
...
############
# HGMD pro
hgmdpro = read.delim('hgmd.tsv')
names(hgmdpro)[2] = "hgmdpro"
optiRVIS = left_join(optiRVIS, hgmdpro, by=c('gene'='gene'))
optiRVIS$hgmdpro[is.na(optiRVIS$hgmdpro)]=0

cols = rainbow(11, start=0.3)

counts = orderpercent(optiRVIS, "oe_lof_upper", c("hgmdpro", "l"))
plot(pchgmdpro ~ pcl, data=counts, type="l", xlab="Percent Bases Covered",
     ylab = "Percent Pathogenic Variants", main="", lwd=1.5, lty="longdash", col=cols[5])
abline(b=1, a=0)

counts = orderpercent(optiRVIS, "pm", c("hgmdpro", "l"))
lines(pchgmdpro ~ pcl, data=counts, lwd=2, col=cols[8])
AUC.pwloeuf = AUC(counts$pcl, counts$pcdenovo)

counts = orderpercent(optiRVIS, "gevir_percentile", c("hgmdpro", "l"))
lines(pchgmdpro ~ pcl, data=counts, lwd=2, col=cols[11])
