// LT 8/09/2020
// Polyphen weighted counts of functional mutations
...
*/
  ...
  for(int i=0;i<obs.rows();i++){
    ...
    // normal approximation of multinomial
    
    Type N = counts.sum();
    
    vector<Type> expected = N * ps;
    
    // standard deviation of the mutinomial + positive constant to allow for overdispersion
    //vector<Type> sds = exp(loga) + sqrt(N * ps * (1 - ps));
    vector<Type> sds = sqrt(N * ps * (1 - ps)) + 0.1;     

    nll -= dnorm(counts, expected, sds, true).sum();
